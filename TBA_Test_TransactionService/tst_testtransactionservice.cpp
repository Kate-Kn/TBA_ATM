...
private slots:
    void test_differentCurrencyCardToCard();
    void test_differentCurrencyCardToAcc();
    void test_canNotTakeTooMarch();
};


TestTransactionService::TestTransactionService() : mockBuilder()
{
...
void TestTransactionService::test_differentCurrencyCardToCard()
{
    int from = 0;
    int to = 4;
    TransactionsCards transaction = TransactionsCards(1, QDate(), "friendly cash");

    User userfrom(mockBuilder.cards[from].user().name(), mockBuilder.cards[from].user().surname());
    Card cardfrom(mockBuilder.cards[from].cardNumber(), mockBuilder.cards[from].balance(), userfrom, mockBuilder.cards[from].currency());
    User userto(mockBuilder.cards[to].user().name(), mockBuilder.cards[to].user().surname());
    Card cardto(mockBuilder.cards[to].cardNumber(), mockBuilder.cards[to].balance(), userto, mockBuilder.cards[to].currency());
    double beforeFrom = cardfrom.balance();
    double beforeTo = cardto.balance();
    try {
        mockBuilder.service->transactionCards(transaction, cardfrom, cardto);
    }  catch (TransactionService::BadTransactionService bs) {
        QVERIFY2(false, bs.diagnose().toStdString().c_str());
    }
    Card cardFromAfterTransaction = mockBuilder.storage->getCard(cardfrom.cardNumber());
    Card cardToAfterTransaction = mockBuilder.storage->getCard(cardto.cardNumber());

    QVERIFY2(beforeTo+1.0*cardto.currency().rateUAH() == cardToAfterTransaction.balance() &&
             beforeFrom-1.0*cardfrom.currency().rateUAH()  == cardFromAfterTransaction.balance(), "Wrong transaction");
}

void TestTransactionService::test_differentCurrencyCardToAcc()
{
    int from = 1;
    int to = 2;
    TransactionsCardAccount transaction = TransactionsCardAccount(10.0, QDate(), "friendly cash");

    User userfrom(mockBuilder.cards[from].user().name(), mockBuilder.cards[from].user().surname());
    Card cardfrom(mockBuilder.cards[from].cardNumber(), mockBuilder.cards[from].balance(), userfrom, mockBuilder.cards[from].currency());
    double beforeFrom = cardfrom.balance();
    try {
        mockBuilder.service->transactionCardAccount(transaction, cardfrom, mockBuilder.accs[to]);
    }  catch (TransactionService::BadTransactionService bs) {
        QVERIFY2(false, bs.diagnose().toStdString().c_str());
    }
    Card cardFromAfterTransaction = mockBuilder.storage->getCard(cardfrom.cardNumber());
    QVERIFY2(beforeFrom-10.0*cardfrom.currency().rateUAH()  == cardFromAfterTransaction.balance(), "Wrong transaction");
}

void TestTransactionService::test_canNotTakeTooMarch()
{
    bool caughtMistake = false;
    int from = 3;
    int to = 9;
    TransactionsCards transaction = TransactionsCards(1, QDate(), "friendly cash");
    ...
    if(!caughtMistake)
        QVERIFY2(false, "Can not take 1 grivna from card with 0 balance");
}


QTEST_APPLESS_MAIN(TestTransactionService)

#include "tst_testtransactionservice.moc"
